...
public class HTTPMsgProcessor {
    ...
    public JSONObject process (String uri, ByteBuf request) throws Exception{
        ...
        switch (uri) {
            case "/feature/add": // 完成
                result = addFeature(request.toString(CharsetUtil.UTF_8));
                break;
            case "/feature/del": // 完成
                result = delFeature(request.toString(CharsetUtil.UTF_8));
                break;
            case "/feature/match":
                result = matchOne(request.toString(CharsetUtil.UTF_8));
                break;
            case "/feature/idel": // 完成
                result = matchMore(request.toString(CharsetUtil.UTF_8));
                break;
    ...
    private JSONObject addFeature(String request){
        JSONObject jObject = JSON.parseObject(request);
        ...
        int appId = jObject.getInteger("app_id");
        String feature = jObject.getString("feature");
        int eyeMode = jObject.getInteger("eyeMode");
    ...
    public JSONObject delFeature(String request){
        ...
        String open_id = jObject.getString("open_id");
        int app_id = jObject.getInteger("app_id");
        int eyeMode = jObject.getInteger("eyeMode");
    ...
    public JSONObject matchOne(String request){
        ...
        String open_id = jObject.getString("open_id");
        int app_id = jObject.getInteger("app_id");
        List<Object> targetList = jObject.getJSONArray("targetIndex").toJavaList(Object.class);
...
//            int featureCount = featureBytes.length / REC_LENGTH;
     ...
     * @return
     */
    private JSONObject describImage(ByteBuf request) throws InvalidProtocolBufferException {
